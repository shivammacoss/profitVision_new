...
const IBPage = () => {
  ...
  const renderDownlineTree = (nodes, level = 0) => {
    ...
    return nodes.map((node, idx) => (
      <div key={node._id || idx} className={`ml-4 border-l pl-4 py-2 ${isDarkMode ? 'border-gray-700' : 'border-gray-300'}`}>
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center ${node.isIB ? 'bg-accent-green/20' : isDarkMode ? 'bg-gray-700' : 'bg-gray-200'}`}>
            <span className={node.isIB ? 'text-accent-green' : isDarkMode ? 'text-gray-400' : 'text-gray-600'}>{node.firstName?.charAt(0) || '?'}</span>
          </div>
          <div>
            <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{node.firstName || 'Unknown'}</p>
            <p className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>{node.email}</p>
          </div>
          <div className="ml-auto text-right">
            <span className={`px-2 py-1 rounded text-xs ${node.isIB ? 'bg-accent-green/20 text-accent-green' : isDarkMode ? 'bg-gray-700 text-gray-400' : 'bg-gray-200 text-gray-700'}`}>
